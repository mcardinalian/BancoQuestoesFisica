...
Um inventor afirmou ter construído uma máquina térmica cujo desempenho atinge acima de 80 \% daquele de uma máquina de Carnot. Sua máquina, que trabalha entre as temperaturas de $\Sexpr{tf}$ °C e $\Sexpr{tq}$ °C, recebe, durante um certo período, $\Sexpr{nota_cient(Qq)}$ cal e fornece, simultaneamente, um trabalho útil de $\Sexpr{nota_cient(trab)}$ J. Dado: 1 cal = 4,2 J.
  ...
  \begin{answerlist}
    ...
    \item A afirmação do inventor é verdadeira (V) ou falsa (F)? Digite \textbf{V} ou \textbf{F}.
    
    \item Qual é o rendimento, em \%, da máquina térmica do inventor? Dê sua resposta com, pelo menos, uma casa decimal.

    \item se o trabalho util da maquina termica fosse exercido sobre um embolo móvel de uma ampola contendo um gás ideal a pressão de $\Sexpr{p}$ N/m², qual seria a variação de volume, em m³, sofrida pelo gás, caso a transformação fosse isobárica? \textit{Dê sua respsta com, pelo menos, uma casa decimal.}
  \end{answerlist}
\end{question}
