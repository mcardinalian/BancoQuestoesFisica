...
questions <- character(22)
solutions <- logical(22)
explanations <- character(22)
aleat = sample(1:22)
  ...
  questions[aleat[11]] <- "Somente com uma fonte fria a 0 °C, seria possível uma certa máquina termica converter integralmente calor em trabalho."
  solutions[aleat[11]] <- FALSE
  explanations[aleat[11]] <- ""
  questions[aleat[12]] <- "Por ser ideal e imaginária, a máquina proposta por Carnot contraria a segunda lei da Termodinâmica."
  solutions[aleat[12]] <- FALSE
  explanations[aleat[12]] <- ""
  questions[aleat[13]] <- "O rendimento da máquina de Carnot depende apenas das temperaturas da fonte quente e da fonte fria."
  solutions[aleat[13]] <- TRUE
  explanations[aleat[13]] <- ""
  questions[aleat[14]] <- "O ciclo de Carnot consiste em duas transformações adiabáticas, alternadas com duas transformações isotérmicas."
  solutions[aleat[14]] <- TRUE
  explanations[aleat[14]] <- ""
  questions[aleat[15]] <- "De acordo com a segunda lei da Termodinâmica, a entropia do Universo tende a diminuir com o tempo."
  solutions[aleat[15]] <- TRUE
  explanations[aleat[15]] <- ""
  questions[aleat[16]] <- "Quando um gás ideal é comprimido rapidamente, a energia interna do gás aumenta."
  solutions[aleat[16]] <- TRUE
  explanations[aleat[16]] <- ""
  questions[aleat[17]] <- " No refrigerador o gás refrigerante remove calor da fonte fria, evaporando-se, e transfere calor à fonte quente, condensando-se."
  solutions[aleat[17]] <- TRUE
  explanations[aleat[17]] <- ""
  questions[aleat[18]] <- "A 1a lei da termodinâmica é uma afirmação do princípio geral da conservação da energia. "
  solutions[aleat[18]] <- TRUE
  explanations[aleat[18]] <- ""
  questions[aleat[19]] <- "Parcela da energia envolvida em um processo irreversível torna-se indisponível para a realização de trabalho. "
  solutions[aleat[19]] <- TRUE
  explanations[aleat[19]] <- ""
  questions[aleat[20]] <- " Nenhuma máquina térmica pode apresentar um rendimento superior ao de uma máquina de Carnot operando entre as mesmas temperaturas."
  solutions[aleat[20]] <- TRUE
  explanations[aleat[20]] <- ""
  questions[aleat[21]] <- "Um refrigerador com a porta aberta, após um longo período, é capaz de esfriar uma cozinha fechada."
  solutions[aleat[21]] <- FALSE
  explanations[aleat[21]] <- ""
  questions[aleat[22]] <- "Em um processo espontâneo, a entropia sempre tende a aumentar."
...
<<echo=FALSE, results=hide, results=tex>>=
answerlist(
  paste("\\textbf{", ifelse(solutions[1:9], "Verdadeiro", "Falso"), "}", sep = ""),
  explanations[1:9])
@
...
%% META-INFORMATION
%% \extype{mchoice}
%% \exsolution{\Sexpr{mchoice2string(solutions[1:9])}}
%% \exname{Q02QuizCal}
